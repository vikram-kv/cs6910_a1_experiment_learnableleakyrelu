...
class NeuralNetwork:
    ...
    def init_parameters(self):
        ...
        leakyalphas = [None for i in range(self.hlayercount + 1)]
        for idx in range(1, self.hlayercount + 1):
            if self.hidden_actfn[idx] == get_act_func_and_deriv('leakyrelu'):
                leakyalphas[idx] = 0.05 * np.ones(self.hidden_sizes[idx])
        return weights, biases, leakyalphas

    # input is of shape -> (in_dim, batchsize); labels is a list of labels of size batchsize
    def forward(self, weights, biases, leakyalphas, input, labels):
        outvalues, actvalues, outderivs = [np.copy(input)], [None], [None]
        for idx in range(1, self.hlayercount + 1):
            cur_act_values = ((weights[idx] @ input).T + biases[idx]).T
            if self.hidden_actfn[idx] != get_act_func_and_deriv('leakyrelu'):
                cur_out_values = self.hidden_actfn[idx][0](cur_act_values)
                cur_out_derivs = self.hidden_actfn[idx][1](cur_out_values)
            else:
                cur_out_values = self.hidden_actfn[idx][0](cur_act_values, leakyalphas[idx])
                cur_out_derivs = self.hidden_actfn[idx][1](cur_out_values, leakyalphas[idx])
            actvalues.append(cur_act_values)
            outvalues.append(cur_out_values)
            outderivs.append(cur_out_derivs)
            input = cur_out_values
        ...
        return outvalues, actvalues, outderivs, fin_act_values, y_pred, loss
    ...
    def backward(self, weights, biases, alphas, labels, outvalues, actvalues, outderivs, fin_act_values):
        ...
        alpha_gradients = [None for i in range(self.hlayercount + 1)]
        ...
        return weight_gradients, bias_gradients, alpha_gradients
    ...
    def test(self, weights, biases, leakyalphas, batches, tname):
        total_count = 0
        total_correct = 0
        total_loss = 0.0
        for X, y in zip(batches[0], batches[1]):
            _, _, _, _, y_pred, loss = self.optimizer.forward(weights, biases, leakyalphas, X, y)
            total_loss += loss
    ...
    def train(self, train_data, val_data, epochs, batchsize, learning_rate, silent=True, log_wandb=False):
        ...
        for e in range(epochs):
            ...
            for X, y in tqdm(zip(train_batches[0], train_batches[1]), total=len(train_batches[0])):
                ...
                outvalues, actvalues, outderivs, fin_act_values, y_pred, loss = self.optimizer.forward(weights, biases, leakyalphas, X, y)
                weight_gradients, bias_gradients, alpha_gradients = self.optimizer.backward(weights, biases, leakyalphas, y, outvalues, actvalues, outderivs, fin_act_values)
                ...
                if (not silent and counter == 100):
                    _, _ = self.test(weights, biases, leakyalphas, val_batches, 'cur validation')
                    counter = 0
            
            print(f'epoch - {e+1}')
            train_acc, train_loss = self.test(weights, biases, leakyalphas, train_batches,'train')
            validation_acc, validation_loss = self.test(weights, biases, leakyalphas, val_batches,'validation')
            if log_wandb:
                wandb.log({'epoch' : e+1, 
