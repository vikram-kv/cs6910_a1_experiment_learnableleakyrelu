...
def agent_code():
    ...
    _, _, _ = nn.train((train_X, train_y), (val_X, val_y), wargs.epochs, wargs.batch_size, wargs.learning_rate, silent=True,log_wandb=True)
...
if __name__ == '__main__':
    ...
    if (args.sid == None):
        sweep_config = {'method' : 'random',
                        ...
                        'metric' : {
                            ...
                            'parameters': {
                                ...
                                'weight_init' : {'values' : ['random', 'xavier'], 'probabilities' : [0.35, 0.65]},
                                'activation_function' : {'values' : ['sigmoid', 'tanh', 'relu'], 'probabilities' : [0.25, 0.25, 0.5]},
                                'momentum' : {'value' : 0.9},
                                'beta' : {'value' : 0.95},
                                'beta1' : {'value' : 0.9},
                                'beta2' : {'value' : 0.999},
        ...
        ter_sweep_config = {'method' : 'random',
                'name' : 'tertiary-sweep',
                'metric' : {
                        'goal' : 'minimize',
                        'name' : 'val_acc'
                    ...
                    'parameters': {
                        ...
                        'num_hlayers' : {'values' : [4, 5]},
                        'hidden_size' : {'values' : [64, 128]},
                        'weight_decay' : {'values' : [0.05, 0.1, 0.25, 0.5]},
                        'learning_rate' : {'values' : [1e-4, 4e-4, 8e-4, 16e-4]},
                        'batch_size' : {'values' : [64, 128]},
                        'loss' : {'value' : 'cross_entropy'},
                        'optimizer' : {'values' : ['sgd', 'momentum', 'nag', 'rmsprop', 'adam', 'nadam'],
                                       'probabilities' : [0.1, 0.15, 0.15, 0.1, 0.3, 0.2]},
        ...
        sweep_id = wandb.sweep(entity='cs19b021', project='cs6910-assignment1',sweep=sweep_config)
        print(sweep_id)
    else:
