...
def gen_parser():
    ...
    parser.add_argument('-e', '--epochs', dest='epochs', default=15, type=int, help='Number of epochs to train neural network')
...
# Not that great in improving accuracy -> achieved ~ 87.5 % on fashion_mnist.
# Hence, in order to speed up sweeps, this was not used. 
...
if __name__ == '__main__':
    ...
    if args.log_wandb != 0:
        ...
        with wandb.init(project=args.wdp, entity=args.wde, config=wandb_config) as run:
            ...
            weights, biases, leakyalphas = nn.train((train_X, train_y), (val_X, val_y), args.epochs, args.batch_size, args.learning_rate, silent=False, log_wandb=True)
    else:
        weights, biases, leakyalphas = nn.train((train_X, train_y), (val_X, val_y), args.epochs, args.batch_size, args.learning_rate, silent=False, log_wandb=False)
    ...
    nn.test(weights, biases, leakyalphas, testbatches, 'test')
